...
login.title('Votação segura com BlockChain!')
...
login.geometry(f'{window_width}x{window_heigth}+{center_x}+{center_y}')
...
label_image_bg = tk.Label(login, image=image_bg)
...
label_logo_grifo = tk.Label(login, image=logo_grifo)
label_logo_grifo.config(background=None)
label_logo_grifo.place(x=160, y=30)

# Criando label do código do aluno
label_codigo_aluno = tk.Label(login, text='Código do aluno', font=('Helvetica', 22))
label_codigo_aluno.place(x=160, y=300)

# Criando input text
input_code = ttk.Entry(login, font=('Helvetica', 24), justify='center', width=10)
input_code.place(x=170, y=400)

# Criando botão submit
btn_font = font.Font(family='Helvetica', size=20, weight='bold')
submit_button = tk.Button(login, text='Submit', width=10, bg='blue', fg='white')
